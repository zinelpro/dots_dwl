...
/*
static void
createkeyboard(struct wl_listener *listener, void *data)
{
	struct wlr_input_device *device = data;
	struct seat *seat = wl_container_of(listener, seat, keyboard_add_listener);
	struct wlr_keyboard *keyboard = wlr_keyboard_from_input_device(device);
	struct xkb_rule_names rules = {0};

	rules.layouts = "us,ru";
	rules.options = "grp:alt_shift_toggle";
...
createkeyboardgroup(void)
{
    ...
    /* Get event loop and set up key repeat timer */
    loop = wl_display_get_event_loop(dpy);
    if (!loop)
        die("failed to get event loop");

    group->key_repeat_source = wl_event_loop_add_timer(loop, keyrepeat, group);
    if (!group->key_repeat_source)
        die("failed to create key repeat timer");

    /* Assign group keyboard to seat */
...
void
autostartexec(void) {
    for (int i = 0; autostart_cmds[i][0]; i++) {
        if (fork() == 0) {
